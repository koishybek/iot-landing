...
def main():
    ...
    # Fix the missing fields caused by scrape.py
    for i, p in enumerate(existing_products):
        if isinstance(p.get("id"), int):
            p["id"] = f"prod_{p['id']}"
        p["priceText"] = p.get("price") if p.get("price") else "Цена по запросу"
        p["category"] = "all"
    ...
    print(f"Saved restored and full data to {JSON_PATH}")
